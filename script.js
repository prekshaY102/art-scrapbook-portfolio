(function () {
  ...
  if (
    ...
    gsap.from(".hero-tape", { opacity: 0, y: -20, duration: 0.8, delay: 1.2 });

    // simple fade/rise reveal for headings & text blocks
    gsap.utils
      .toArray(
        ".about-heading,.about-body,.about-edit-note,.chapter-tab,.chapter-heading,.chapter-sub,.contents-heading,.process-heading,.contact-heading,.contact-sub",
      )
      .forEach(function (el) {
        gsap.fromTo(
          el,
    ...
    document.querySelectorAll(".chapter-grid").forEach(function (grid) {
      ...
      grid.querySelectorAll(".polaroid").forEach(function (el, i) {
        ...
        gsap.set(el, { opacity: 0, y: 36, scale: 0.88, rotation: start });
        gsap.to(el, {
          opacity: 1,
          y: 0,
          scale: 1,
    ...
    (function () {
      var rotations = [-6, 4, -3];
      document
        .querySelectorAll(".stickers-row .sticker")
        .forEach(function (el, i) {
          var rest = rotations[i % rotations.length];
          var start = rest + (Math.random() * 10 - 5);
          gsap.set(el, { opacity: 0, scale: 0.6, rotation: start });
          gsap.to(el, {
            opacity: 1,
            scale: 1,
            rotation: rest,
    ...
    document
      .querySelectorAll(".process-row .process-card")
      .forEach(function (el, i) {
        var rest = i % 2 === 0 ? -2 : 2;
        var start = rest + (Math.random() * 10 - 5);
        gsap.set(el, { opacity: 0, y: 36, scale: 0.9, rotation: start });
        gsap.to(el, {
          opacity: 1,
          y: 0,
          scale: 1,
    ...
    // about photo: simple pop-in (rotation matches the -3deg set inline in the HTML)
    var aboutPhoto = document.querySelector(".about-visual .polaroid");
    if (aboutPhoto) {
      gsap.set(aboutPhoto, { opacity: 0, y: 36, scale: 0.9, rotation: 6 });
      gsap.to(aboutPhoto, {
        opacity: 1,
        y: 0,
        scale: 1,
